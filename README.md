# Darryl Beatty — Product Manager Portfolio

Personal portfolio site showcasing product management work across consumer platforms, operational tooling, and market intelligence.

## Live Site

[darrylbeatty.github.io](myportfolio)

## What's Inside

- **Home** — Hero section, key metrics, four project case studies, core strengths
- **Case Studies** — Community Engagement Engine, Automated Reporting Dashboard, Social-to-Strategy Research, Dual-View Project Dashboard
- **About** — Background, tools, disciplines, certifications, contact

## Built With

- HTML, CSS, JavaScript (single-page application with client-side routing)
- Playfair Display + DM Sans (Google Fonts)
- No frameworks, no dependencies, no build step

## Brand

- Dark theme with Signal Blue (#3B82F6) accents
- Brand guidelines by Darryl Beatty, 2025

## Contact

- Email: dbeattyj@gmail.com
- LinkedIn: [linkedin.com/in/darryl-beatty-jr](https://www.linkedin.com/in/darryl-beatty-jr/)
